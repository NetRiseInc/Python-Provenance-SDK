from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field

T = TypeVar("T", bound="OrganizationsItem")


@_attrs_define
class OrganizationsItem:
    """
    Attributes:
        confidence (int): A confidence rating of this attribution as a percentage (0-100)
        methods (list[str]): A list of method names that attributed this quality to the entity
        name (str): The display name of the organization
        repository_url (str): The url for the organization's repository
    """

    confidence: int
    methods: list[str]
    name: str
    repository_url: str
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        confidence = self.confidence

        methods = self.methods

        name = self.name

        repository_url = self.repository_url

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "confidence": confidence,
                "methods": methods,
                "name": name,
                "repository_url": repository_url,
            }
        )

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        confidence = d.pop("confidence")

        methods = cast(list[str], d.pop("methods"))

        name = d.pop("name")

        repository_url = d.pop("repository_url")

        organizations_item = cls(
            confidence=confidence,
            methods=methods,
            name=name,
            repository_url=repository_url,
        )

        organizations_item.additional_properties = d
        return organizations_item

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
